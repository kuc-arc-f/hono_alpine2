...
//
export const AlpineTest3: FC<{ items: any[] }> = (props: { items: any[] }) => {
  const timeStamp = Date.now();
  return (
    <Layout>
      <div>
        <h1 class="text-4xl font-bold">AlpineTest3</h1>
        <hr class="my-2" />
        <div id="root"></div>
        <script defer src="https://cdn.jsdelivr.net/npm/alpinejs@3.13.1/dist/cdn.min.js"></script>
        {/* alpine  */}
        {html`
        <h3 class="text-3xl font-bold">x-for + D1</h3>
        <hr class="my-2" />
        <div class="mb-5" x-data="data">
            <template x-for="item in items" :key="item.id">
              <div x-data="{ ...{ item }}">
                ...
                , <span class="mb-3 font-normal text-gray-700" x-text="item.createdAt"></span>
                <hr class="my-2" />
              </div>
            </template>
            <!-- button --> 
        ...
        <hr class="my-2" />
        {/* JS */}
        {html`<script src="/js/csr/alpine3.js?${timeStamp}"></script>`}
